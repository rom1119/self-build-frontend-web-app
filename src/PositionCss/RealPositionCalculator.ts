...
export default class RealPositionCalculator
{
    ...
    public updateProps()
    {
        ...
        Vue.nextTick(() => {
            var accessor = this.tag.selectedSelector

            if (!this.tag.hasAbsolute && !this.tag.hasFixed ) {
                return
            }

            if (this.tag.currentCssAccessor.hasCssProperty(TopCss.PROP_NAME)) {
                this.topUnit = this.tag.currentCssAccessor.getProperty(TopCss.PROP_NAME).getUnit()
                this.top = this.tag.currentCssAccessor.getProperty(TopCss.PROP_NAME).getClearValue()
            }
            if (this.tag.currentCssAccessor.hasCssProperty(LeftCss.PROP_NAME)) {
                this.leftUnit = this.tag.currentCssAccessor.getProperty(LeftCss.PROP_NAME).getUnit()
                this.left = this.tag.currentCssAccessor.getProperty(LeftCss.PROP_NAME).getClearValue()
            }
    ...
    {
        if (this.tag.currentCssAccessor.hasCssProperty(RightCss.PROP_NAME)) {
            this.rightUnit = this.tag.currentCssAccessor.getProperty(RightCss.PROP_NAME).getUnit()
            this.right = this.tag.currentCssAccessor.getProperty(RightCss.PROP_NAME).getClearValue()
        }
    }

    public updateBottomProps()
    {
        if (this.tag.currentCssAccessor.hasCssProperty(BottomCss.PROP_NAME)) {
            this.bottomUnit = this.tag.currentCssAccessor.getProperty(BottomCss.PROP_NAME).getUnit()
            this.bottom = this.tag.currentCssAccessor.getProperty(BottomCss.PROP_NAME).getClearValue()
        }
    }
    ...
    recalculateValRight(val) {
        ...
        var marginRight = this.tag.marginRealFetcher.fetchPropValue(MarginRightCss.PROP_NAME)
        // throw Error('b')
        if (!this.nearPositionalTag || (this.nearPositionalTag && !this.nearPositionalTag.getHtmlEl())) {
            // @ts-ignore
    ...
    recalculateRealValBottom(val) {
        ...
        if (this.tag.parent) {
            ...
            if (this.tag.parent.getHtmlEl()) {
                // @ts-ignore
                bottomParent = this.tag.parent.getHtmlEl().getBoundingClientRect().bottom
            }
