<template >
  <div class="content-item__elem_container">
    ...
    <div class="content-item__elem" v-context-menu="cmXpos">
      <select-radial-gradient-direction-unit-menu
        :propertyUnit="value.xPosUnit"
        @changePropUnit="
          ($event) => {
            value.xPosUnit = $event;
            change();
          }
        "
        :ref="cmXpos"
      ...
      <label :for="'posX'">
        ...
        {{ value.getXPosValue() }}
        <select
          v-if="isNamedPosXUnit"
          name="posX"
          @change="change"
          v-model="value.xPos"
          id="posX"
        >
          <option v-for="posX in xPosList" :value="posX" :key="posX">
            {{ posX }}
          </option>
        </select>
        <input
    ...
    <div class="content-item__elem" v-context-menu="cmYpos">
      <select-radial-gradient-direction-unit-menu
        :propertyUnit="value.yPosUnit"
        @changePropUnit="
          ($event) => {
            value.yPosUnit = $event;
            change();
          }
        "
        :ref="cmYpos"
      ...
      <label :for="'posY'">
        ...
        {{ value.getYPosValue() }}
        <select
          v-if="isNamedPosYUnit"
          @change="change"
          name="posX"
          v-model="value.yPos"
          id="yPos"
        >
          <option v-for="posY in yPosList" :value="posY" :key="posY">
            {{ posY }}
          </option>
        </select>
        <input
