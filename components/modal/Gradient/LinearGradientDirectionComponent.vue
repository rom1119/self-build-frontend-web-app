<template xmlns:v-slot="http://www.w3.org/1999/XSL/Transform">
    ...
    <div class=" content-item__elem_container"
        >
        <div v-if="value != null" class="content-item__elem"
                v-context-menu="cmNameTextShadowBlur"
                >
            <select-linear-gradient-direction-unit-menu :propertyUnit="value.directionUnit" @changePropUnit="($event) => {value.directionUnit = $event; change();}" :ref="cmNameTextShadowBlur" />
            ...
            <label :for="'textShadowBlur-'" style="display: inline-flex;">
                ...
                <select v-if="isNamedUnit" name="direction" @change="change" v-model="value.direction" id="linear-direction">
                    <option v-for="direction in directions" :value="direction" :key="direction"> {{ direction }} </option>
                </select>
                <template v-else>
                
                    <input  type="number" @dblclick.stop.prevent="" :step="inputStep" @input="change" style="width: 40px;" class="input-text" v-model="value.direction" :maxSize="maxSize" />
...
import Linear from '../../../src/Css/Animation/timingFunction/impl/Linear';
    ...
    export default class LinearGradientDirectionComponent extends Vue {
        ...
        cmNameTextShadowBlur = Math.floor(Math.random() * 1000000000).toString() + 'direction-linear-gradient'
